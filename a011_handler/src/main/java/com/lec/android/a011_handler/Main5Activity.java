...
public class Main5Activity extends AppCompatActivity {
    ...
    class BackgroundTask extends AsyncTask<Integer, Integer, Integer> {
        ...
        //백그라운드 작업 . 반드시 구현!
        @Override
        protected Integer doInBackground(Integer... integers) { // 가변매개변수, integers는 Integer[]
            ...
            return backValue1; // onPostExecute에 넘어가는 값

            // doInBackground 에서 시간이 많이 걸리는 '통신' 작업이나 복잡한 연산 작업등을 (비동기로) 수행케 해야 한다
        }

        //백그라운드 작업 도충 (여러번) 호출 가능, 진행상황 업데이트, 중간중간 UI 업데이트시 사용
        @Override
