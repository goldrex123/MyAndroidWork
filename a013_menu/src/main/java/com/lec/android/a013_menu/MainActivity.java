...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.menu_main1, menu); // memu 리소스(XML) 로부터 Menu 생성
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        Log.d("myapp", "onPrepareOptionsMenu() - 옵션메뉴가 화면에 보여질 때마다 호출");


        //메뉴가 보여질때마다 로그인, 로그아웃이 enable/disable 토글되어 보이기
        if (bLog){ //로그인 상태
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        Log.d("myapp", "onOptionsItemSelected - 메뉴항목을 클릭 했을 때 호출됨");
