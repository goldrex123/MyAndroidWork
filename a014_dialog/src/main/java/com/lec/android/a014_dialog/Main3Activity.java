        ...
        switch (id) {
            case DIALOG_DATE:
                DatePickerDialog dpd = new DatePickerDialog(
                        this, // 현재 화면의 제어권자
                        new DatePickerDialog.OnDateSetListener() { // 날짜 설정후 Dialog 빠져나올때 호출되는 콜백
                            @Override
            ...
            case DIALOG_TIME:
                TimePickerDialog tpd = new TimePickerDialog(
                        this,
                        new TimePickerDialog.OnTimeSetListener() { // 시간 값 선택후 호출되는 콜백
                            @Override
                            public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
                                Toast.makeText(getApplicationContext(), hourOfDay + "시" + minute + "분 선택"
                                        , Toast.LENGTH_SHORT).show();
                            }
                        }, 17, 21, false // 기본값 시, 분 등록
                            // is24HourView
                            // true : 24시간 제
                            // false : 오전 , 오후 항목 표시 12시간제
                );
                tpd.show();
                return tpd;
        }
