...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button btn1 = findViewById(R.id.btn1); // R -> resource 값이 모여 있는 것
...
//                return false; // false를 리턴하면 이벤트가 click 까지간다
                return true; //true를 리턴하면 이벤트가 long click 으로 소멸 (consume) 된다
            }
        });

        //방법 3: 람다 익스프레션 사용하기
              // AndroidStudio 의 셋팅 필요!
        btn3.setOnClickListener((v) -> {
            Log.d("myapp", "버튼 3가 클릭되었다.");
            tvResult.setText("버튼 3가 클릭됨");
        ...
        btnDec.setOnClickListener(new View.OnClickListener() {
            ...
            @Override
            public void onClick(View v) {
                float size1 = tvResult.getTextSize();
                Log.d("myapp", "- 버튼이 클릭되었습니다");
                tvResult.setTextSize(0, size1-1);
            }
        });
    }

    // 방법 1: 레이아웃 xml의 onxxx 에 지정
    ...
    // 방법 5 : 액티비티가 implement 한 것을 사용하는 방법
    @Override
    public void onClick(View v) {
